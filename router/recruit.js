...
router.get('/', function(req, res) {
    if (req.query.filter && req.query.filter == 'checkSurvived') {
        ...
        if (recruit_wait == 1) {
            ...
            res.render('recruit/recruit-ing', {'n_th': n_th, 'due_month': dates.due_month, 'due_day': dates.due_day}); // '모집 종료일' 설정, 지원중 노출은 '모집 종료일' 자정 전 까지
        } else if (time < '2018-08-19 00:00:00') {
            res.render('recruit/recruit-fin', {'due_day': dates.due_day}); // '1차 발표일' 설정, 모집종료 노출은 '1차 발표일' 자정 전 까지
